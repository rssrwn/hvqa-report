...
\begin{enumerate}
  ...
  Rule~\ref{rule:rule-count} attempts to minimises the number of rules which appear in the learnt hypothesis, while Rule~\ref{rule:rule-size} attempts to minimise the total weight of the rules in the hypothesis. Both of these rules, however, are given lower priority than the true positive and true negative rules, since we always prefer a more accurate solution, even if it is longer.
\end{enumerate}
...
\begin{itemize}
  ...
  \item Finally, \textit{change colour} is defined in terms of the $changed(p, \mathit{before}, \mathit{after}, \mathit{id}, i)$ predicate, which says that property $p$ has value $\mathit{before}$ in frame $i$, but changed to value $\mathit{after}$ in frame $i+1$.
  \begin{equation}
    \begin{split}
  ...
  The \textit{change colour} effect is then simply defined as follows:
  \begin{equation}
