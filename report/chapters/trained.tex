...
\begin{enumerate}
  ...
  \item A number of helper ASP rules which convert convert object data in the form of the $obj$ and $\textit{$<$prop$>$}\_mapping$ predicates into the $holds$ predicate which is used to answer the questions. These two helper rules are as follows:
  \begin{gather}
  ...
  \item The final part of the ASP program is the set of weak constraints used to find the optimal mapping. Firstly we define the helper rule, $mapping$, which is used to collate both the colour and rotation property values for each cluster identifier:
  \begin{equation}
    mapping(\mathit{Cluster}, \mathit{Col}, \mathit{Rot}) \text{ :- } \begin{multlined}[t]
      colour\_mapping(\mathit{Cluster}, \mathit{Col}), \\
      rotation\_mapping(\mathit{Cluster}, \mathit{Rot}).
    \end{multlined}
  \end{equation}
