...
% In the remainder of this section we provide an English description for the task which the \textsc{OptimiseFeature} function must solve, and, in particular, what the search space for the optimisation looks like.
...
\begin{enumerate}
  ...
  \item Then, for each possible number of rules, each rule can choose to use at most one of $\{ acc_{r_1},...,acc_{r_m} \}$. Therefore, for each previous node $n$ which allows $R$ rules, for every rule $r$ such that $r <= R$, $m+1$ child nodes are added to $n$, one node for each choice of accumulated operations.
...
% The case where the algorithm is left to decide how many rules are required - where the number of rules to learn is part of the search - is more complex. In this case there is one set of accumulated operations for each rule learnt so far, and, during the next feature optimisation, the algorithm can choose any one of the accumulated operation sets for each rule being learnt, or none. Algorithm~\ref{algo:optimise} gives a high-level overview of the steps required to find the optimal hyposthesis - the set of rules which correctly answers the most questions - for each action.
...
This section outlines the details of that program.


% TODO mention how effects are modelled separately
