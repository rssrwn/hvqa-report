...
These ASP rules are found using a greedy search algorithm. The algorithm is run three times; once for each each action. Before outlining this algorithm, we first define the terminology used as follows:
\begin{itemize}
...
\begin{equation}
  ...
  \left\{
  	\begin{array}{ll}
  		\sigma(f_{v_{i}}, f_{v_{i+1}}) := f_{v_i} = c_i, f_{v_{i+1}} = c_j \textit{ for each $(c_i, c_j)$} \\
  ...
  \label{eqn:trained-close-def}
\end{equation}
\textit{Where $(c_i, c_j)$ loops over all possible combinations of feature values.}
