...
\begin{enumerate}
  ...
  \item Then, for each possible number of rules, each rule can choose to use at most one of $\{ acc_{r_1},...,acc_{r_m} \}$. Therefore, for each previous node $n$, which allows $R$ rules, for every rule $r_k$ such that $r_k <= R$, $m+1$ child nodes are added to $n$, one node for each choice of accumulated operations, including none.
