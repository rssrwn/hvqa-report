...
\begin{itemize}
  \item Firstly, the search space for the optimisation is constructed using choice rules. For each cluster identifier $c_i$, the following two choice rules are added:
  \begin{gather}
    1\{ colour\_mapping(c_i, col_1) ; ... ; colour\_mapping(c_i, col_n) \}1. \\
    1\{ rotation\_mapping(c_i, rot_1) ; ... ; rotation\_mapping(c_i, rot_m) \}1.
  \end{gather}
  ...
  \item Secondly, the ASP rules corresponding to the questions and the ASP facts corresponding to the answers to those questions are added to the program. For the tuple $(p_i, v_i, t_i, a_i)_{\textit{$<$i$>$}}$ extracted by the question and answer parsing component from the $i^{th}$ QA pair, the following rule and fact are added if $v_i$ is not $Null$, where $p_{v_i}$ is the property which corresponds to property value $v_i$:
  \begin{gather}
    answer(\textit{$<$i$>$}, p_i, V) \text{ :- } \begin{multlined}[t]
      holds(p_{v_i}(v_i, \mathit{Id}), \textit{$<$i$>$}), \\
      holds(p_i(V, \mathit{Id}), \textit{$<$i$>$}), obj(\mathit{Id}, \_, \textit{$<$i$>$}).
    \end{multlined} \\
    answer(\textit{$<$i$>$}, p_{v_i}, V) \text{ :- } \begin{multlined}[t]
      holds(p_{v_i}(V, \mathit{Id}), \textit{$<$i$>$}), \\
      holds(p_i(v_i, \mathit{Id}), \textit{$<$i$>$}), obj(\mathit{Id}, \_, \textit{$<$i$>$}).
    \end{multlined} \\
    expected(\textit{$<$i$>$}, p_i, a_i). \\
    expected(\textit{$<$i$>$}, p_{v_i}, v_i).
  \end{gather}
  ...
  \begin{gather}
    answer(\textit{$<$i$>$}, colour, V) \text{ :- } \begin{multlined}[t]
      holds(rotation(down, \mathit{Id}), \textit{$<$i$>$}), \\
      holds(colour(V, \mathit{Id}), \textit{$<$i$>$}), obj(\mathit{Id}, \_, \textit{$<$i$>$}).
    \end{multlined} \\
    answer(\textit{$<$i$>$}, rotation, V) \text{ :- } \begin{multlined}[t]
      holds(rotation(V, \mathit{Id}), \textit{$<$i$>$}), \\
      holds(colour(white, \mathit{Id}), \textit{$<$i$>$}), obj(\mathit{Id}, \_, \textit{$<$i$>$}).
    \end{multlined} \\
    expected(\textit{$<$i$>$}, colour, white). \\
    expected(\textit{$<$i$>$}, rotation, down).
  \end{gather}
  ...
  \begin{gather}
    answer(\textit{$<$i$>$}, p_i, V) \text{ :- } holds(p_i(V, \mathit{Id}), \textit{$<$i$>$}), obj(\mathit{Id}, \_, \textit{$<$i$>$}). \\
    expected(\textit{$<$i$>$}, p_i, a_i).
  \end{gather}

  % TODO Add obj(Id, Label, Question) predicate description (may want to put this above since obj is used above)

  % TODO Add mapping rules

  % TODO Add optimisation rules

  % TODO update number in intro to this list
\end{itemize}
