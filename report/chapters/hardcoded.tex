...
In the remainder of this section we diverge from $\mathcal{AL}$ orthodoxy in order to model effects. Effects are defined by ASP rules with a combination of actions and $\mathcal{AL}$ fluents in the body.
...
Firsly, for the \textit{eat a fish} (an example of which is shown in Figure~\ref{fig:eat-fish}) and \textit{eat a bag} effects, the following ASP rules are used:
\begin{equation}
  \begin{split}
...
%   move(Id) \textbf{ causes } \neg colour(Old, Id) \textbf{ if } colour(Old, Id), change_colour(Old, New, Id)
% \end{gather*}
