...
We also need to ensure the octopus' colour is updated when it comes close to rock, and that we keep track of any \textit{change colour} effects that occur. To do this we, firstly, use the following helper predicate:
\begin{equation}
  \begin{split}
...
\begin{equation}
  occurs\_effect(change\_colour(Octo), I) \text{ :- } change\_colour(Old, New, Octo, I).
\end{equation}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.6\textwidth]{branch-and-bound}
  \caption{Part of an example action search tree. Each action is given as a pair of $(action\_name, timestep)$. Red crosses and dashed lines show pruned branches.}
  \label{fig:branch-and-bound}
\end{figure}

All of the above rules, as well as the ASP encoding of the $\mathcal{AL}$ model and the observed data, are run in a single ASP program which attempts to find the optimal set of actions for the video. Speed is a genuine concern for this optimisation; the ASP program has to evaluate $4^{32}$ action combinations, which could take a very long time. The ASP optimiser, however, uses optimisation strategies which can decrease the total search time. These algorithms include an ASP-specific extension of the branch-and-bound algorithm~\cite{asp:branch-and-bound}, which allows the optimiser to prune branches of the search tree if the branch is provably sub-optimal. Part of an example search tree for the action optimisation is shown in Figure~\ref{fig:branch-and-bound}. For most videos in the evaluation dataset, the ASP optimisation is able to run in under 5 seconds, however, when the observed data is noisier, the algorithm has to explore many more answer sets, which takes significantly longer. This is discussed further in Chapter~\ref{chapter:evaluation}.
...
\begin{figure}[t]
  ...
  \caption{An example of an error in the object detection. Two purple octopuses are detected in this frame. This leads to competion for the identifier of the `real' octopus.}
