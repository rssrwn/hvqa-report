...
% Outline an AL system description for OceanQA environment. Mention that the ASP encoding for an AL sys desc is provided in appendix X.

% Ensure the ASP rules in appendix are same as event model ASP.

% Types

% rotation(R, Id), position(P, Id), class(C, Id), colour(C, Id) are inertial fluents
% close(Id1, Id2) is defined fluent
% eat_a_fish(Id), eat_a_bag(Id), change_colour(Id) are effect type

% Domain independent
% Same as AL encoding
% Also, CWA for inertial fluents in initial frame
% -holds(F, 0) :-
%   fluent(inertial, F),
%   not holds(F, 0).
...
% Move:
% move causes position(@new_pos(Pos, Rot), Id) if position(Pos, Id), rotation(Rot, Id)
% move causes -position(position(Pos, Id))     if position(Pos, Id)

% Rotate clockwise
% rotate_clockwise causes rotation(@new_rot(Rot, "rotate_clockwise") if rotation(Rot, Id)
% rotate_clockwise causes -rotation(Rot, Id)                         if rotation(Rot, Id)

% Rotate ac
% rotate_anticlockwise causes rotation(@new_rot(Rot, "rotate_anticlockwise"), Id) if rotation(Rot, Id)
% rotate_anticlockwise causes -rotation(Rot, Id)                         if rotation(Rot, Id)


As mentioned above, the optimal set of actions is found by comparing the observed features of the environment with an internal $\mathcal{AL}$ model of the environment. In this section we outline the $\mathcal{AL}$ system description for the OceanQA environment. This system description can then be written in ASP using the encoding provided in Appendix~\ref{appendix-al}.

Firstly, we outline the types involved in the $\mathcal{AL}$ system description, which are as follows:
\begin{itemize}
  \item Properties, including class and position, are modelled as \textit{inertial fluents} - their values can change as a direct result of the actions taken. Notice, however, that the number of possible position values is very large - $256^4$, hence, rather than allowing all possible values, we only include position values that have been observed in the video, all other values are not modelled.
  ...
  \item We create a new type, called \textit{effect}, to model the effect events. As with actions, each effect takes a single argument and are written as $\textit{$<$effect$>$}(\textit{$<$id$>$})$.
\end{itemize}

The domain independent rules for the OceanQA environment as as outlined in Appendix~\ref{appendix-al}, with the following addition:
\begin{equation}
...
Finally, we outline the domain dependent $\mathcal{AL}$ statements for the all three actions in OceanQA environment:
\begin{itemize}
