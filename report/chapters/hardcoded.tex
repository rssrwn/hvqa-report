...
The relations component of the hardcoded model contains a hand-written algorithm which determines whether two objects are close or not. For a given video, this algorithm is applied to every pair of objects in every frame of the video in order to list all instances of the relation. The algorithm for determining the closeness of two objects is given in Algorithm~\ref{algo:close}, and is described textually below. The object arguments of this algorithm are given in symbolic form, rather than as raw pixel matrices. This means the algorithm is heavily reliant on accurate information from the object detector. Although the closeness algorithm doesn't make use of the property component's extracted features, other algorithms for determining binary relations may require these.
