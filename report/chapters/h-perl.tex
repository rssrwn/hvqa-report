...
We outline a novel paradigm for solving the VideoQA problem. This approach merges deep learning and logic-based machine learning and inference methods in an attempt to learn the concepts required to answer the questions. We name this approach: The Hybrid Property, Event and Relation Learner (H-PERL). Section~\ref{section:h-perl-model} outlines the architecture of an H-PERL model, while Section~\ref{section:common-components} discusses the implementation of the H-PERL components which are common to all models presented in the subsequent chapters.


\section{H-PERL Models}
...
\begin{figure}
  ...
  \caption{An overview of the H-PERL architecture for VideoQA. Green, red and blue shading indicate components which extract features at different levels of abstraction. Grey shading indicates the `core' components of the architecture.}
...
\begin{itemize}
  \item For an object with identifier \textit{$<$id$>$} in frame \textit{$<$frame$>$}, the object's properties, rotation and class (all referred to as $\textit{$<$property$>$}$), each with value\footnotemark $\textit{$<$value$>$}$, are represented as follows:
  \begin{equation}
  ...
  \begin{gather}
    ...
    \text{occurs\_effect}(\textit{$<$effect$>$}(\textit{$<$id$>$}), \textit{$<$frame$>$})
  \end{gather}
\end{itemize}

\footnotetext{As described in Chapter~\ref{chapter:dataset}, the value of an object's position is given as $(x1, y1, x2, y2)$, where $(x1, y1)$ is the top left corner of the object, and $(x2, y2)$ is the bottom right.}
...
\begin{enumerate}
  \item A set of pre-made, non-core components.
...
\begin{enumerate}
  ...
  \begin{equation}
    \begin{split}
      answer(\textit{$<$question$>$}, \textit{$<$property$>$}, Val) \text{ :- } & \textsc{AspObj}(\textit{$<$object$>$}, Id, \textit{$<$frame$>$}), \\
      & obs(\textit{$<$property$>$}(Val, Id), \textit{$<$frame$>$}).
    \end{split}
  \end{equation}
  ...
  \begin{equation}
    \begin{split}
      answer( & \textit{$<$question$>$}, \mathit{Prop}, \mathit{Before}, \mathit{After}) \text{ :- } \\
      & changed(\mathit{Prop}, \mathit{Before}, \mathit{After}, \mathit{Id}, \textit{$<$frame$>$}), \\
      & exists(\mathit{Id}, \textit{$<$frame$>$}+1), \\
      & \textsc{AspObj}(\text{octopus}, \mathit{Id}, \textit{$<$frame$>$}).
    \end{split}
  \end{equation}
  The answer string is then constructed as follows:
  $$\text{Its $\mathit{Prop}$ changed from $\mathit{Before}$ to $\mathit{After}$}$$

  \item Repetition count questions resemble the following template:
  $$\text{How many times does the octopus \textit{$<$event$>$}?}$$
  The ASP rule for a repetition count question with question number \textit{$<$question$>$} is as follows:
  \begin{equation}
    \label{eqn:repetition-count}
    \begin{split}
      answer(\textit{$<$question$>$}, Num) \text{ :- } & event\_count(\textit{$<$event$>$}, Id, Num), \\
      & \textsc{AspObj}(\text{octopus}, Id, \_).
    \end{split}
  \end{equation}
  In Rule~\ref{eqn:repetition-count} an anonymous frame number variable is supplied to the \textsc{AspObj} function since we don't care which frames the octopus is in; we simply need to fetch its identifier. The answer string is then simply the value of $Num$.

  \item Repeating action questions take the following form:
  $$\text{What does the octopus do \textit{$<$num$>$} times?}$$
  The ASP rule for these questions is constructed as follows:
  \begin{equation}
    \begin{split}
      answer(\textit{$<$question$>$}, Event) \text{ :- } & event\_count(Event, Id, \textit{$<$num$>$}), \\
      & \textsc{AspObj}(\text{octopus}, Id, \_).
    \end{split}
  \end{equation}
  The answer string is given as the value of the $Event$ variable.

  \item Finally, state transition questions have the following template:
  $$\text{What does the octopus do immediately after \textit{$<$event\_noun$>$} [for the \textit{$<$nth$>$} time]?}$$
  After \textit{$<$event\_noun$>$} is converted to event form, which we denote \textit{$<$event$>$}, and the string \textit{$<$nth$>$} is converted to a number, denoted \textit{$<$occ$>$} (this is simply 1 if the occurrence string is omitted), the ASP rule can be constructed as follows:
  \begin{equation}
    \label{eqn:state-transition}
    \begin{split}
      answer(\textit{$<$question$>$}, Action) \text{ :- } & action(Action), \\
      & occurs\_event(Action, Id, Frame+1), \\
      & event\_occurrence(\textit{$<$event$>$}, Id, Frame, \textit{$<$occ$>$}), \\
      & \textsc{AspObj}(\text{octopus}, Id, Frame).
    \end{split}
  \end{equation}
  In Rule~\ref{eqn:state-transition} the type predicate, $action$, is used to ensure that the answer is an action. If this predicate is omitted from the body the answer may incorrectly be given as an effect. Once the answer has been found, the answer string is given as the value of the $Action$ variable.
\end{enumerate}

Hardcoding the question-answering rules does, of course, bring a number of disadvantages; namely that the entire component must be rewritten for every new environment, and that the rules may be very complex and time consuming to write. It also introduces potential for human bias or errors in the rules, meaning that the ASP rules may not work as well as intended. However, although it is not part of this project, there is a possibility of learning these rules using Inductive Logic Programming. This would allow the component to be considerably more versatile and potentially more accurate. Chapter~\ref{chapter:conclusion} discusses this idea further.
