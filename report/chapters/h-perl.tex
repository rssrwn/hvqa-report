...
\begin{enumerate}
  ...
  After \textit{$<$event\_noun$>$} is converted to event form, which we denote \textit{$<$event$>$}, and the string \textit{$<$nth$>$} is converted to a number, denoted \textit{$<$occ$>$}, the ASP rule can be constructed as follows:
  \begin{equation}
