...
The object detection component uses the Faster R-CNN algorithm~\cite{cnn-uses:faster-r-cnn}, as described in Chapter~\ref{chapter:background}. Faster R-CNN was chosen over YOLO~\cite{yolo} as it has been shown to be more accurate~\cite{cnn-uses:yolo-v3}. YOLO's main advantage or Faster R-CNN is its speed~\cite{cnn-uses:yolo-v3}. However, since the video frames are fairly small and simple, and since VideoQA is not a real-time task, speed is not the highest priority.
