...
The object descriptor shown in~\ref{eqn:asp_obj} are used in the body of the ASP rule templates and allows the QA system to find an object with class \textit{$<$class$>$}, identifier \textit{$<$id$>$}, and, optionally, with property \textit{$<$property$>$} taking value \textit{$<$property\_value$>$}, in frame \textit{$<$frame\_idx$>$}. We refer to the object descriptor in~\ref{eqn:asp_obj} as:
...
\begin{equation}
  \begin{split}
    ...
    \textit{$<$asp\_obj(}\text{fish, Id1, colour, blue, I+1}\textit{)$>$} & = holds(class(fish, Id1), I), holds(colour(blue, Id1), I)
  \end{split}
\end{equation}

The ASP rule templates for each question type are described as follows:
\begin{enumerate}
  \item For property questions
