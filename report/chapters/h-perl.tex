...
\begin{enumerate}
  ...
  \begin{equation}
    ...
    \end{split}
  \end{equation}
  Then the answer string is simply the \textit{value} argument to the $answer$ predicate.
  ...
  \begin{equation}
    ...
    \end{split}
  \end{equation}
  The answer string is then either yes or no, depending on the result contained in the $answer$ predicate.

  \item Action questions have the following form:
  $$\text{Which action occurred immediately after frame \textit{$<$frame$>$}?}$$
  We create one of the following ASP rules for each action, including the `nothing' action:
  \begin{equation}
    \begin{split}
    \end{split}
  \end{equation}
