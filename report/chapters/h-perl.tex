...
If \textit{$<$object$>$} does not contain a property value, the property predicate in~\ref{eqn:asp_obj} is omitted. In order to simplify the ASP rule templates, which follow, we denote the predicates in~\ref{eqn:asp_obj} as:
$$\textsc{AspObj}(\textit{$<$object$>$}, \textit{$<$id$>$}, \textit{$<$frame$>$})$$

In the following, we show how the ASP rule is constructed using the information from the parsed question, for each question type:
\begin{enumerate}
  \item Property questions have the following template:
  $$\text{What \textit{$<$property$>$} was the \textit{$<$object$>$} in frame \textit{$<$frame\_idx$>$}?}$$

  Following question parsing, the ASP rule for question number \textit{$<$question$>$} is constructed as follows:
  \begin{equation}
    \begin{split}
      answer(\textit{$<$question$>$}, \textit{$<$property$>$}, Value) \text{ :- } & \textsc{AspObj}(\textit{$<$object$>$}, Id, \textit{$<$frame\_idx$>$}), \\
      & obs(\textit{$<$property$>$}(Value, Id), \textit{$<$frame\_idx$>$}).
    \end{split}
  \end{equation}

  \item Relations questions ...
\end{enumerate}
