...
An H-PERL pipeline assumes that all of the information in an environment which is required to answer the questions can be modeled using: objects; binary relations between objects; and events (which occur due to an object) between two consecutive frames in the video. For many environments, simple environments (like our OceanQA dataset) in particular, this assumption holds. However for many other VideoQA environments, particularly those set in the real-world, this assumption may not be suitable. For example, we may not always be able to extract objects from a video (as in the case of abstract nouns), and yet information on these objects may still be required to answer the questions. Additionally, the H-PERL structure is not capable of modelling relations between objects with an arity larger than 2.
...
\begin{figure}
  ...
  \end{subfigure}
  \caption{Potential modifications to the H-PERL pipeline for solving other tasks}
...
%   \includegraphics[width=\textwidth]{vqa-pipeline}
%   \includegraphics[width=\textwidth]{captioning-pipeline}
%   \caption{Potential modifications to the H-PERL pipeline for solving other tasks. \textbf{Top}: VQA pipeline. \textbf{Bottom}: Image captioning pipeline.}
%   \label{fig:other-pipelines}
% \end{figure}
