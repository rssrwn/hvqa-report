...
In order to conduct the evaluation, we used the \textit{PyCocoTools}\footnote{Available at: https://github.com/cocodataset/cocoapi} library, which implements the metrics used in the COCO object detection challenge. Table~\ref{table:detector-aps} presents the average precision values, for a range of IoU thresolds, achieved by our detector on the OceanQA validation dataset.
