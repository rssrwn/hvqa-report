...
Having said this, however, it is also worth keeping in mind that the artificial mistakes added to the detection are a very blunt; real object detector errors are much more nuanced. For example, although state-of-the-art object detectors can achieve a mAP of almost $80\%$ on some datasets~\cite{cnn-uses:faster-r-cnn}, that doesn't mean that $20\%$ of objects are missed entirely. Rather, it may mean that objects are detected correctly but with low confidence scores, or that objects are detected but the their bounding boxes are slightly wrong, or even that object are detected with the wrong class. For this reason, we cannot directly interpolate the results of these robustness experiments to the results we would expect to see H-PERL models achieve on real-world data. However, these experiments do make clear that H-PERL models are heavily reliant on a high-performing object detection component.
