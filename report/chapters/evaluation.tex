...
Despite being made of neural networks, the properties components are both fairly fast. Both components took approximately 18 seconds to label all of the objects in the testing data with property values. As with the detector, both property component implementations are deemed adaptable, since they can simply be retrained for a new environment. However, as with the detector, it is worth noticing that the hardcoded properties component requires extra supervision, making it less adaptable than its trained counterpart.
...
\begin{enumerate}
  ...
  \item As alluded to earlier, the trained model is significantly faster than either of the hardcoded implementations. This is entirely down to the difference in speed between the hardcoded and trained events components.
\end{enumerate}
...
As mentioned previously, the hardcoded model cannot be easily ported to another environment. This is mostly due to its relations and events components, but also because it makes use of the full-data version of the dataset for training its properties component. Other environments will not be able to guarantee that a similar type of dataset is available. The trained model, on the other hand, is more adaptable to new environments, since its core components can be re-trained using a different dataset fairly easily. Both models, however, make use of manually engineered object tracking, question parsing and QA system components. These components would likely prove difficult or time-consuming to rewrite for new environments.
...
In this section we only deal with a single type of object detection error, where the detector misses an object completely. To implement this, we take the full-data version of the 200 OceanQA test videos and remove each object with probability $p$. We run tests with four different values for $p$: $0$, $0.1$, $0.2$ and $0.3$. A probability of $0$ means that the full-data version of the test data is unchanged. Figures~\ref{fig:hardcoded-accs} and~\ref{fig:trained-accs} show how the accuracies of the hardcoded and trained models change with $p$. The pink bar in each case shows the accuracy of the standard H-PERL model, where the trained object detector is used. The results for the error correcting version of the hardcoded model are not shown since they are very similar to those of the standard hardcoded model.
...
Figure~\ref{fig:model-accs} shows the accuracies of the models when a 20 second timeout was applied to the event detection components. This is particularly important for the hardcoded model and helps to explain the stark difference in performance for higher values of $p$. Due to the timeout, the hardcoded event detector was often unable to search through the entire space required to find the optimal set of actions. As mentioned in Chapter~\ref{chapter:hardcoded}, the hardcoded event detector is likely to suffer from this problem when the input data is noisy, which is exactly the sort of condition these robustness tests are trying to emulate. Table~\ref{table:robustness-timings}, which provides the robustness test timings for each model, shows the problem in more detail.

\begin{table}[b!]
  \centering
  \begin{tabular}{ |c|c c c c| }
    \hline
    \diagbox{\textbf{Model:}}{\textbf{$p$:}} & \textbf{$0$} & \textbf{$0.1$} & \textbf{$0.2$} & \textbf{$0.3$} \\
    \hline
    \textbf{Hardcoded}      & 47.2  & 47.1   & 98.9   & 121.6 \\
    \textbf{Hardcoded (EC)} & 268.8 & 1600.0 & 2398.5 & 2347.9 \\
    \textbf{Trained}        & 254.5 & 1564.4 & 2309.2 & 2249.9 \\
    \hline
  \end{tabular}
  \caption{Time taken in seconds for each model to complete the robustness test evaluation for different values of $p$, the probability of an object being missed.}
  \label{table:robustness-timings}
\end{table}

\pagebreak

Although the performance of the trained model, during the robustness tests, is significantly better than that of the hardcoded model, the tests show that both models are heavily reliant on highly accurate object detection. Going from perfect object detection down to $90\%$ detection leads to a $24.7\%$ drop in accuracy, from $99.7\%$ to $75\%$. Given that many object detectors trained on real-world datasets have significantly poorer performance than our OceanQA detector, means that more work may need to be done to improve this situation before H-PERL can be applied to real-world environments.

Having said this, however, it is also worth keeping in mind that the artificial mistakes added to the detection are very blunt; real object detector errors are much more nuanced. For example, although state-of-the-art object detectors can achieve a mAP of almost $80\%$ on some datasets~\cite{cnn-uses:faster-r-cnn}, that doesn't mean that $20\%$ of objects are missed entirely. Rather, it may mean that objects are detected correctly but with low confidence scores, or that objects are detected but the their bounding boxes are slightly wrong, or even that objects are detected with the wrong class. For this reason, we cannot directly interpolate the results of these robustness experiments to the results we would expect to see H-PERL models achieve on real-world data. However, these experiments do make clear that H-PERL models are heavily reliant on a high-performing object detection component.
