...
As well as videos, VideoQA datasets must also contain question-answer pairs (QA pairs). Answers to questions are usually the only form of direct supervison given to a VideoQA model, and, unlike other supervised learning datasets, VideoQA datasets can contain lots of sparsely labelled data. For example, our dataset can contain at most 31 octopus actions per video, but only a small number of these may be `labelled' by QA pairs. Properties of objects are even more sparsely labelled, since there can be as many as 512 object instances in a single video, but perhaps only a single QA pair. For this reason, VideoQA tasks can resemble `semi-supervised' tasks, where a model is given partially labelled data, but asked to produce all labels.
...
\begin{enumerate}
  ...
  \item Changed-property questions require the model to reason about how a property of the octopus changes from one frame to the next. The model can assume that only a single property changes after the frame mentioned in the question. Approximately $78\%$ of these questions ask about the colour of the octopus, while the remaining $22\%$ ask about the rotation. The grammar is as follows:
  \begin{equation}
    \begin{split}
...
As alluded to already, an environment specification, like the one shown in Equation~\ref{eqn:env-spec}, requires that objects, properties, relations, actions and effects are all discrete, and does not allow the modelling of continuous variables. For many environments this requirement would be debilitating to any model attempting to learn the the environment's underlying rules. This may have to be treated as a trade-off for using hybrid models (Chapter~\ref{chapter:conclusion} discusses trade-offs further). On the other hand, this is one of the first pieces of work which explores the use of hybrid models in VideoQA tasks; future work may be able to overcome this problem.
