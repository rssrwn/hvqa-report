...
\footnotetext{The CLEVRER dataset~\cite{dataset:clevrer} does meet these requirements and would have been a good candidate for this project. Unfortunately it was published in March 2020, six months after the project began.}
...
As well as videos VideoQA datasets must also contain question-and-answer pairs (QA pairs). Answers to questions are the usually only form of direct supervison given to a VideoQA model, and, unlike other supervised learning datasets, VideoQA datasets can contain lots of data which is sparsely labelled. For example, our dataset contains 31 octopus actions per video, but only a small number of these may be `labelled' by QA pairs. Properties of objects are even more sparsely labelled, since there can be as many as 512 object instances in a single video.

\begin{equation}
  \label{eqn:spec-grammar}
  \begin{split}
    \textbf{object} & ::= [ \textbf{ property\_val } ] \textbf{  class } \\
    \textbf{property\_val} & ::= \textbf{ rotation\_val } | \textbf{ colour\_val } \\
    \textbf{rotation\_val} & ::= \text{upward-facing } | \text{ right-facing } | \text{ downward-facing } | \text{ left-facing } \\
    \textbf{colour\_val} & ::= \text{red } | \text{ blue } | \text{ purple } | \text{ brown } | \text{ green } | \text{ silver } | \text{ white } \\
    \textbf{class} & ::= \text{ octopus } | \text{ fish } | \text{ bag } | \text{ rock } \\
    \textbf{property} & ::= \text{ rotation } | \text{ colour } \\
    \textbf{relation} & ::= \text{close} \\
    \textbf{event} & ::= \textbf{action } | \textbf{ effect } \\
    \textbf{action} & ::= \text{move } |  \text{ rotate clockwise } | \text{ rotate anti-clockwise } \\
    \textbf{effect} & ::= \text{change colour } | \text{ eat a fish } | \text{ eat a bag }
  \end{split}
\end{equation}

The TODO dataset contains ten QA pairs per video, sampled randomly from seven question-types. The grammar for each QA pair is presented in extended Backus-Naur form (EBNF) in Equation~\ref{eqn:qa-grammar}, while Equation~\ref{eqn:spec-grammar} formalises the grammar of the dataset's objects, relations and events, which was implicitly described in the Section~\ref{section:dataset-videos}.

\begin{equation}
  \label{eqn:qa-grammar}
\end{equation}
